...
def _get_memory_manager() -> MemoryManager:
    return MemoryManager(db=DatabaseManager(), vector_store=VectorStore(), extractor=None)
...
registry.register(ToolDefinition(
    ...
    parameters=[
        ...
        ToolParameter("memory_type", "string", "Type of memory (fact, preference, etc)", required=False, enum=["fact", "summary", "preference", "goal", "project", "habit", "routine", "deadline", "observation"]),
        ToolParameter("importance", "number", "Importance score from 0.0 to 1.0", required=False),
        ToolParameter("conversation_id", "string", "Optional conversation ID", required=False)
    ],
...
registry.register(ToolDefinition(
    ...
    parameters=[
        ToolParameter("memory_id", "string", "The ID of the memory to delete")
    ],
